...
class _CartItemWidgetState extends State<CartItemWidget> {
  ...
  double _calculatePriceWithVariation({required CartModel cartModel, required double? discount, required String? discountType}) {
    ...
    if(newVariation) {
     ...
     // price = price + _calculateAddonPrice(cartModel) + (PriceConverter.convertWithDiscount(cartModel.item!.price!, discount, discountType, isFoodVariation: false)! * cartModel.quantity!);

      // price = price * cartModel.quantity!;
    //  price = price * cartModel.quantity!;
      if(price!=0)
        {
          price = price;
...
//      price =  _calculateAddonPrice(cartModel) + (PriceConverter.convertWithDiscount(cartModel.item!.price!, discount, discountType, isFoodVariation: true)! * cartModel.quantity!);
